...
set "GAME_NAME=%1"
set "CPU_TYPE=--cpu 6502"
rem set "CPU_TYPE=-t nes"
...
for %%S in (asm/*.s) do %CC65_CA% %CPU_TYPE% -I lib -o obj/%%~nS.o asm/%%S

rem Compile C files
for %%C in (src/*.c) do %CC65_CC% %CPU_TYPE% -I include --add-source -Oisr -o tmp/%%~nC.s src/%%C

rem Compile temp assemblie files
for %%S in (tmp/*.s) do %CC65_CA% %CPU_TYPE% -I lib -o obj/%%~nS.o tmp/%%S

rem Build .o file list
set "OBJ_FILES="
for %%F in (obj/*.o) do call set "OBJ_FILES=obj/%%F %%OBJ_FILES%%"

rem set "LD_CONFIG=-t nes"
set "LD_CONFIG=-C config/nrom_256_horz.cfg"
set "LD_DBG=--dbgfile bin/%GAME_NAME%.dbg"
rem set "LD_DBG="

rem Link all .o files into .nes file
%CC65_LD% %LD_CONFIG% %LD_DBG% -L %CC65_LIB% -o bin/%GAME_NAME%.nes %OBJ_FILES% || goto :fail
